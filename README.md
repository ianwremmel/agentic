...
This repository is a [Claude Code plugin marketplace](https://code.claude.com/docs/en/plugin-marketplaces). It packages plugins that extend Claude Code with skills, agents, commands, and hooks tuned for day-to-day engineering work. The catalog ships `dispatch`, a multi-ticket project orchestrator, and `land`, a stateless skill that takes a single pull request to completion. Skills, agents, and hooks are being migrated from a prior repo; plugin directories are scaffolded and ready for content to land.
...
Install a plugin by name, scoped to this marketplace:
...
| [`land`](plugins/land) | Bring one pull request to completion: draft it, drive CI and reviews, iterate, and monitor until it merges or closes. Started from a PR URL, a ticket URL, or a freeform prompt. Stateless — no database, no orchestration. |
