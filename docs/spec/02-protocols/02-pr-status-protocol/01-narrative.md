...
full. Summaries are generated by a cheap/fast model invocation, lazily — only
the first time an item settles into non-actionable — and then persisted
alongside the cache file. Once written a summary is never regenerated, even if
the item's body later changes, so a quiet PR pays no re-summarization cost
