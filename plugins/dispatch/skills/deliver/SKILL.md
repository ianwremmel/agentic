...
**Read the actual `team_mode` value before you act on it — never infer the mode from the documented default.** It comes from `CLAUDE_PLUGIN_OPTION_TEAM_MODE` (the `team_mode` userConfig). If you have not read that variable this session, you do not know the mode and must not assume one: the `false` default below is what the config means *when unset*, not license to skip the read. Mode picks which states are reachable and which gate clears the draft, so an unverified assumption silently drives the wrong lifecycle (e.g. clearing draft straight to the team on a CODEOWNERS-guarded repo that wanted a private pre-review first). Resolve it explicitly at startup alongside the operator identity.
...
- **The `actionable` attribute is the sole authority.** Drive every gate and lifecycle decision off `actionable="true|false"` — nothing else. A non-actionable item carries a `reason=` token (`resolved`, `agent-artifact`, `agent-terminal-reply`, `acked`) saying *why* it's suppressed, and may carry a `<summary>` for human context. **Neither is a license to re-derive actionability.** In particular, never reopen a non-actionable item because its `<summary>` prose reads as unresolved ("remains unaddressed", "no reply"): the summary describes the thread's *content*, not its *resolution*, so an item you already terminal-tagged will summarize as if the reviewer's point still stands. That contradiction is expected — the flag is correct, the prose is just color. If you find yourself about to redo work because a summary looked open, stop: check the `reason=` instead. (This is the same trap as bypassing `pr-status` — substituting your own reading for the classifier's.)
- **A requested review is in-flight, not absent.** `<review-requests>` lists reviewers whose review GitHub has *requested but not yet received*. While it lists a reviewer — especially a `mode="bot"` one (Copilot et al.) — that review is still landing; its inline threads can post minutes after the request. An empty or stable thread set while a request is outstanding is **not** convergence. Keep polling on the reviewer cadence until the request clears: the reviewer drops out of `<review-requests>` and reappears under `<reviews>` with a state, and its threads (if any) show up actionable. Calling the PR "clean" while a request is still outstanding is the premature-convergence failure.
