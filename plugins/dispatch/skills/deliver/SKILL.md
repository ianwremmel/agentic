...
| Actionable `<comment>` or `<thread>` (gates 4–5 fail) | Reply per §2.1 with **either** a commit link describing what changed **or** a one-line dismissal rationale. Apply terminal signal. **Never resolve the thread** — resolution is the initiator's call (§2.1); your terminal signal already suppresses re-evaluation. |
