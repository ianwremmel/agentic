...
2. **Judge the goal** against that evidence — the goal, not the ticket count.
   Every ticket can be `verified` and the goal still missed. Gaps, regressions,
   and work that shipped short of its aims are all follow-up.
...
4. **Record the outcome** as a comment on the milestone's review artifact — a
   Linear project update, a GitHub Milestone closure comment, an Asana
   milestone-task comment. Say whether the goal was achieved, on what evidence,
   and list every follow-up ticket filed.
...
`awaiting-input`; you are re-dispatched later, and a fresh run finds your open
request and waits again.

## Report

Standalone: report to the session. Dispatched: hold the heartbeated
milestone-keyed lock and write `outcome.json` as your final action
([`orchestrate/reference.md`](../orchestrate/reference.md#dispatch)) —
`{ "key": "milestone:<id>", "outcome": "recorded" | "awaiting-input" | "failed", "followups": [], "detail": "…" }`.
