...
import {buildResource, readPluginVersion, SERVICE_NAME} from './resource.mts';
...
async function writeManifest(body: string): Promise<URL> {
...
describe('readPluginVersion', () => {
  it('reads the version the plugin manifest declares', async () => {
    ...
    // `0.0.0` fallback.
    assert.equal(await readPluginVersion(), await readDeclaredVersion());
  });

  it('reads a distinctive version out of a manifest', async () => {
    assert.equal(
      await readPluginVersion(
        await writeManifest('{"version": "9.8.7-probe"}')
      ),
      '9.8.7-probe'
    );
  });
  ...
  it('falls back to 0.0.0 when the manifest is not there', async () => {
    assert.equal(
      await readPluginVersion(new URL('file:///nonexistent/plugin.json')),
      '0.0.0'
    );
  });
  ...
  it('falls back to 0.0.0 when the manifest is truncated', async () => {
    assert.equal(
      await readPluginVersion(await writeManifest('{"version": "0.1')),
      '0.0.0'
    );
  });
  ...
  it('falls back to 0.0.0 when the version is not a string', async () => {
    assert.equal(
      await readPluginVersion(await writeManifest('{"version": 3}')),
      '0.0.0'
    );
  });
});
...
describe('buildResource', () => {
  it('names the service and the version the manifest declares', async () => {
    const {attributes} = await buildResource();
    ...
    assert.deepEqual(
      ...
      {name: SERVICE_NAME, version: await readDeclaredVersion()}
    );
  });

  it('leaves the detected attributes to the SDK', async () => {
    // One that already carried process.* and host.* would be doing the
    // detectors' job, and one carrying more under service.name would stop
    // OTEL_SERVICE_NAME from winning.
    assert.deepEqual(Object.keys((await buildResource()).attributes).sort(), [
      ATTR_SERVICE_NAME,
      ATTR_SERVICE_VERSION,
    ]);
  });
