---
name: deliver
description: Drive a single code change to merge through a draft PR — CI, reviews, iteration, monitor until close. The unit of work is "land this change," given as a PR URL, a ticket URL, or a freeform prompt.
---
...
`deliver` takes one unit of work, given as any of:

- **A PR URL** — resume it. Reuse its worktree and branch and skip the open
  sequence (see [Setup](#setup) → Resume).
- **A ticket URL** — read the ticket for context with whatever tracker tools
  the session has, implement the change on a fresh branch, then open the PR.
  `deliver` bundles no tracker adapter: it reads the ticket best-effort and
  never blocks on tracker writes.
- **A freeform prompt** — implement the described change on a fresh branch,
  then open the PR.

Once the PR exists the loop below is identical regardless of entry point.
...
- **Reply to every reviewer item** — commit link or dismissal rationale;
  silence is non-conforming. Reserve a bare one-line dismissal for a bot's
  false-positive nit; a human reviewer's point earns a fix or a substantive
...
Adaptive, not fixed. Tune within the schedule from what you observe this run —
no history persists between runs. **Never poll faster than once per minute.**
...
is invoked directly or as a subagent.
...
- **Ending the turn before a lifecycle terminal** — returning early for "no work
  ...
  caller around mid-lifecycle re-invocation.

At a lifecycle terminal (or a caught operator "stop") perform this run's cleanup
(remove any worktree you created, plus any lock, label, or status write your
invocation specified). Abnormal exits (API errors, OOM, reaping) are the
caller's stale-state sweep's job, not a substitute for this discipline.
